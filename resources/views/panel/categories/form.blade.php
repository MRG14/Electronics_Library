...
<html>
    <head>
      ...
      <!-- Title: Start -->
      <title>Panel - {{ $category ? 'Ubah Kategori' : 'Tambah Kategori' }} | {{ config('app.name', 'Digital Library') }}</title>
      <!-- Title: End -->
      ...
      <section id="panel-section"
        ...
        <main class="w-full h-full bg-white overflow-y-scroll rounded-lg drop-shadow-lg flex flex-col p-3 lg:p-5">
          ...
          <form class="w-full lg:w-1/2 flex flex-col gap-4"
            ...
            action="{{ $category ? route('categories.update', $category->id) : route('categories.create') }}">
            @csrf
            ...
            <div class="text-sm lg:text-base w-full flex flex-col gap-1 mt-4">
              ...
              <input id="title-field"
                ...
                placeholder="Judul Kategori Anda"
                required/>
            </div>
            ...
            <div class="text-sm lg:text-base w-full flex flex-col gap-1">
              ...
              <input id="image-field"
                ...
                accept="image/*"
                class="w-full outline-none p-3 rounded-lg border placeholder:text-black/30"
                placeholder="Foto Kategori Anda"
                @if(!$category) required @endif/>
            ...
            <button type="submit" 
              ...
              {{ $category ? 'Update Kategori' : 'Kirim Kategori' }}
            </a>
          </form>
        </main>
        <!-- Panel Content: End -->
